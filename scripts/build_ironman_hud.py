...
def _pill(unit: str) -> str:
    ...
    m = {"usd":"$", "pct":"%", "x":"x", "count":"#", "score":"score"}
...
def _fallback_fcf_yield_from_dcf(ticker: str):
    ...
    Looks at export/CANON_{T}/{T}_DCF.json with inputs.fcf_ttm and inputs.market_cap_used.
    """
    try:
        from pathlib import Path
        import json
        ...
        fcf = float(d.get("inputs", {}).get("fcf_ttm") or 0.0)
...
<div class="wrap">
  ...
  <div class="grid">
      <div class="card wide">
        ...
        <div class="row" style="margin-top:10px;"><div><b>Free cash flow (TTM)</b></div><div>{_fmt_usd(fcf)}{_pill("usd")}</div></div>
        ...
        <div class="row" style="margin-top:10px;"><div><b>FCF yield</b></div><div>{_fmt_pct(fcf_y)}{_pill("pct")}</div></div>
        <div class="small">Cash return vs what you pay for the whole company. {_interp_fcf_yield(fcf_y)}.</div>
